...
class Pagina
{
    ...
    private function getPaginaSql($id)
    { 
    $sql = "SELECT * from paginas as p join concessionarias as c on p.concessionaria = c.id_concessionaria join pagina_site as ps on p.id_pagina = ps.pagina_id join sites as s on ps.site_id = s.id_site left join pagina_contato as pc on p.id_pagina = pc.pagina_id_ left join contatos as ct ON pc.contato_id = ct.id_contato where id_pagina = {$id}";
        $result = $this->instancia->query($sql);
        return $result;
    }
    ...
    // public function getAllPagesSql()
    // {
    //     $sql = "SELECT * from paginas as p join concessionarias as c on p.concessionaria = c.id_concessionaria join pagina_site as ps on p.id_pagina = ps.pagina_id join sites as s on ps.site_id = s.id_site left join pagina_contato as pc on p.id_pagina = pc.pagina_id_ left join contatos as ct ON pc.contato_id = ct.id_contato";
...
$dados = $t->exec($_GET['v']);
// echo '<pre>';   
print_r(json_encode(convert_to_utf8_recursively($dados),JSON_UNESCAPED_SLASHES));
// echo '<pre>';
